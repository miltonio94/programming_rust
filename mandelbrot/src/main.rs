...

    {
        ...
        crossbeam::scope(|spawner| {
            for (i, band) in bands.into_iter().enumerate() {
                ...
                let bands_bounds = (bounds.0, height);
                let bands_upper_left = pixel_to_point(bounds, (0, top), upper_left, lower_right);
                let bands_lower_right =
                    pixel_to_point(bounds, (bounds.0, top + height), upper_left, lower_right);

                spawner.spawn(move |_| {
                    render(band, bands_bounds, bands_upper_left, bands_lower_right);
                });
            }
        })
        .unwrap();
